...
export default function QuizPreview() {
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      try {
        ...
        const uniqueChapters = [...new Set(data.map((row) => row.chapter))];
  ...
  if (loading) return (
    <div className="flex justify-center items-center min-h-screen">
      <p className="text-xl font-bold text-indigo-600 animate-pulse">Loading chapters...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-center text-blue-700 mb-10">Preview Chapters</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {chapters.map((chapter, idx) => (
              ...
              onClick={() =>
                navigate("/quiz", {
                  state: {
                    ...
                    chapter,
                    isPreview: true,
                    duration: 5, // shorter preview duration
                  },
                })
              }
            ...
            >
              ...
              <p className="text-center mt-2 text-sm">Click to preview this quiz</p>
            </div>
          ))}
        </div>
      </div>
